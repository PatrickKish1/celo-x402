...
export default function DiscoverPage() {
  ...
  const [viewMode, setViewMode] = useState<"resources" | "servers">(
    "resources"
  );
  ...
  const refreshData = async () => {
    try {
      ...
      const validServices = (liveServices || []).filter(
  ...
  const filteredServices = services.filter((service) => {
    ...
    const searchText = `${service?.metadata?.name || ""} ${
      service?.metadata?.description || ""
    } ${service?.resource || ""}`.toLowerCase();
        ...
        <main className="flex-grow py-12 px-4">
          <div className="container mx-auto text-center">
            ...
            <p className="text-gray-600 font-mono">
              Fetching live data from x402 Bazaar...
            </p>
          </div>
        </main>
      </div>
        ...
        <div className="container mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-xl font-mono text-gray-700 max-w-3xl mx-auto mb-8">
              Browse and explore available x402 APIs from the live Bazaar. Find
              the perfect service for your needs with transparent pricing and
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 overflow-hidden">
            {paginatedServices.map((service) => {
              ...
              const price = x402Service.formatUSDCAmount(
                primaryPayment?.maxAmountRequired || "0"
              );
              const rating = x402Service.getServiceRating(service);

              // Generate service ID for URL
              const serviceId = service?.resource
                ? generateServiceId(service.resource)
                : "unknown";
              ...
              const fullDescription =
                service?.accepts?.[0]?.description ||
                service?.metadata?.description ||
                "Professional x402 API service with instant payment integration.";
              const isExpanded = expandedCards.has(serviceId);
              const { text: displayDescription, isTruncated } = isExpanded
                ? { text: fullDescription, isTruncated: false }
                : truncateText(fullDescription);
              ...
              return (
                ...
                >
                  <div className="mb-4">
                    <h3
                      className="text-xl sm:text-2xl font-bold font-mono tracking-wide mb-1 overflow-hidden text-ellipsis break-words"
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                    ...
                    <div>
                      ...
                      <div className="text-gray-600">
                        {primaryPayment?.network?.toUpperCase() || "BASE"}
                      </div>
                    </div>
                    <div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        window.location.href = `/discover/${serviceId}/test`;
                      }}
          ...
          {/* Refresh Button */}
          <div className="text-center mt-8">
            <button onClick={refreshData} className="retro-button bg-gray-100">
              REFRESH SERVICES
            </button>
          </div>
        </div>
